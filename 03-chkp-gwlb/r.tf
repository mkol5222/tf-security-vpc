...
aws ec2 create-route --route-table-id ${local.nat_rt_b} --destination-cidr-block 10.250.6.0/24 --vpc-endpoint-id ${local.gwlbe_b}
aws ec2 create-route --route-table-id ${local.nat_rt_c} --destination-cidr-block 10.250.7.0/24 --vpc-endpoint-id ${local.gwlbe_c}
AAA
}
output "cmd_fw_off" {
    value = <<BBB
